...
export const errorHandlerMiddleware: ErrorRequestHandler = (unknownError, request, response, _nextFunction) => {
  ...
  return response.status(500).json({ success: false, error: { code: "INTERNAL_SERVER_ERROR", message: "The request could not be completed.", ...(backendEnvironmentConfiguration.NODE_ENV !== "production" ? { details: String(unknownError) } : {}) } });
};
